...
class SomfyProtexialElementsCard extends HTMLElement {
  ...
  _render() {
    ...
    if (this.config.device_id && !this._registryLoading) {
      console.debug("[Somfy Protexial Elements Card v1.4.0]", {
        ...
        ` : elements.length ? (() => {
          ...
          <div class="empty">
            ${this.config.device_id
              ? this._t(
                  "Aucun élément Somfy avec attributs de diagnostic n’a été trouvé.",
                  "No Somfy equipment with diagnostic attributes was found."
                )
              : this._t(
                  "Sélectionne l’appareil Somfy dans l’éditeur.",
                  "Select the Somfy device in the editor."
                )}
          </div>
        `}
